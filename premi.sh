...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/Sabdo-Dadi/vps/main/"
...
print_success "NOOBZVPNS BY SABDO PALON"
}
...
function menu(){
	...
	wget ${REPO}limit/m-noobz
	wget -q -O /usr/bin/enc "${REPO}/limit/encrypt"
	...
	mv m-noobz /usr/local/sbin/m-noobz
...
cat >/etc/cron.d/xp_all <<-END
	...
	END
cat >/etc/cron.d/daily_auto_backup <<-END
		SHELL=/bin/sh
		PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
		0 22 * * * root /usr/local/sbin/backer
	END
